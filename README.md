...
* **Simulation Engine:** The game logic [**(baseGame)**](Mastermind/src/baseGame/) and AI solver [**(ai)**](Mastermind/src/ai/) were written in **Java**. The solver uses a strategy of randomly selecting a guess from the universe of remaining valid codes. The [**(GUI)**](Mastermind/src/GUI/) uses Java's swing library. Download the [**latest release**](https://github.com/philMarcus/Mastermind/releases/download/v1.5/Mastermind.1.5.AI.Batches.jar) to play and use all features.
